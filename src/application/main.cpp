...
static void parseargs(int argc, char** argv, popsift::Config& config, string& inputFile) {
    ...
    modes.add_options()
        ...
        "Choice of descriptor extraction modes. Default is OpenCV-like horizontal scan, computing only valid points (loop), grid extracts only useful points but rounds them, igrid extracts useful points and relies on linear interpolation textures. Options are: loop, grid, igrid")
